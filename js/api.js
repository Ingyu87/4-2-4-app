...
export async function getAIFeedback(noteOrQuestion, stage, articleType) {
    ...
    if (isQuestionStage) {
        ...
        prompt = `
            ...
            **평가:** [학생이 만든 질문이 [글 종류]와 [활동 단계]에 알맞은 질문인지 1-2 문장으로 칭찬하거나 격려]
            - 질문이 잘 만들어졌다면: "아주 훌륭합니다! 설명하는 글을 읽을 때 가장 중요한 목표인 **정보 파악**과 **중심 내용 찾기**를 성공적으로 수행했습니다."
            - 질문이 부족하다면: "좀 더 구체적인 질문을 만들어 보면 읽기 과정을 점검하는 데 도움이 될 거예요."
            
            **활동 분석:** ${questionFocusInfo}
            
            **개선 제안:** [피드백 지침] 2번을 바탕으로, 이 질문을 더 깊게 만들거나, 추가하면 좋을 질문의 예시를 1가지 구체적으로 제안
            ${questionExample}
        `;
    } else {
        // 내용 작성 활동 피드백
        ...
        prompt = `
            ...
            **평가:** [학생이 작성한 내용을 1-2 문장으로 칭찬하거나 격려]
            - 내용이 잘 작성되었다면: "아주 훌륭합니다! 설명하는 글을 읽을 때 가장 중요한 목표인 **정보 파악**과 **중심 내용 찾기**를 성공적으로 수행했습니다."
            - 내용이 부족하다면: "좀 더 구체적으로 적어보면 읽기 과정을 점검하는 데 도움이 될 거예요."
            
            **활동 분석:** ${activityFocusInfo}
            
            **개선 제안:** [피드백 지침] 2번을 바탕으로, 학생의 활동 내용을 더 깊게 만들 수 있는 구체적인 조언 1-2문장을 제안
            ${activityExample}
        `;
    }
