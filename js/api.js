...
export async function getAIFeedback(noteOrQuestion, stage, articleType) {
    ...
    if (isQuestionStage) {
        ...
        prompt = `
            당신은 초등학교 4학년 담임 선생님 역할의 AI입니다. 학생이 만든 '질문'을 분석하고 조언해야 합니다.
            '사랑하는 OOO' 같은 호칭은 절대 사용하지 말고, 학생이 만든 질문과 답의 내용을 객관적으로 분석합니다.
            무조건 칭찬하지 말고, 실제 질문과 답의 내용을 정확히 확인하여 분석해주세요.

            [학생의 활동]
            - 글 종류: ${articleType}
            - 활동 단계: ${stageMap[stage]}
            - 학생이 만든 질문과 답: "${noteOrQuestion}"

            [피드백 지침]
            반드시 다음 형식으로 작성해주세요 (평가 섹션은 제외):
            
            **활동 분석:** 
            학생이 만든 질문과 답의 내용을 실제로 확인하여 다음을 분석해주세요:
            ${questionFocusInfo}
            
            - 질문이 [글 종류]와 [활동 단계]에 맞는지 구체적으로 분석
            - 답의 내용이 질문에 적절히 답하고 있는지 확인
            - 질문과 답이 글의 핵심 내용을 파악하는 데 도움이 되는지 평가
            - 부족한 점이 있다면 구체적으로 지적
            
            **개선 제안:** 
            활동 분석을 바탕으로, 이 질문을 더 깊게 만들거나, 추가하면 좋을 질문의 예시를 1가지 구체적으로 제안해주세요.
            ${questionExample}
        `;
    } else {
        // 내용 작성 활동 피드백
        ...
        prompt = `
            당신은 초등학교 4학년 담임 선생님 역할의 AI입니다. 학생의 '활동 내용'을 분석하고 조언해야 합니다.
            '사랑하는 OOO' 같은 호칭은 절대 사용하지 말고, 학생이 작성한 내용을 객관적으로 분석합니다.
            무조건 칭찬하지 말고, 실제 작성 내용을 정확히 확인하여 분석해주세요.
            ...
            **활동 분석:** 
            학생이 작성한 내용을 실제로 확인하여 다음을 분석해주세요:
            ${activityFocusInfo}
            
            - 작성 내용이 [글 종류]와 [활동 단계]에 맞는지 구체적으로 분석
            - 내용이 충실한지, 구체적인지 확인
            - 부족한 점이 있다면 구체적으로 지적
            
            **개선 제안:** 
            활동 분석을 바탕으로, 학생의 활동 내용을 더 깊게 만들 수 있는 구체적인 조언 1-2문장을 제안해주세요.
            ${activityExample}
        `;
    }
...
export async function getAIReportEvaluation(journey, articleType) {
    ...
    const prompt = `
        당신은 초등학교 4학년 담임 선생님 역할의 AI입니다. 학생의 읽기 활동 보고서를 객관적이고 엄격하게 평가해야 합니다.
        '사랑하는 OOO' 같은 호칭은 절대 사용하지 말고, 학생의 실제 활동 내용을 정확히 확인하여 평가합니다.
        무조건 칭찬하지 말고, 실제 활동 내용을 기준으로 엄격하게 평가해주세요.
        
        [성취 기준]
        [4국02-03] 질문을 활용하여 글을 예측하며 읽고 자신의 읽기 과정을 점검한다.

        [평가 기준 - 엄격하게 적용]
        - 잘함: 읽기 전, 중, 후의 과정에 알맞은 질문을 활용해 읽기 과정을 되돌아보며, 자신의 읽기 과정을 **스스로 조정**할 수 있다. 
          * 필수 조건: 피드백을 받고 v2로 수정한 내용이 v1보다 명확하게 나아졌는지 확인
          * 읽기 중 활동: 질문과 답이 글의 핵심 내용을 실제로 파악하는 데 도움이 되는지 확인 (단순히 질문만 만들었다고 잘함이 아님)
        - 보통: 읽기 전, 중, 후의 과정에 알맞은 질문을 활용해 읽기 과정을 되돌아볼 수 있다.
          * v1 활동을 충실히 완료했는지 확인
          * 질문과 답이 있지만 내용이 부족하거나 핵심을 놓친 경우
        - 노력 요함: 자신의 읽기 과정을 되돌아보는 데 도움이 필요하다.
          * 활동 내용이 매우 부족하거나, 질문과 답이 글의 핵심과 관련이 없는 경우
          * "몰라요", "같아요", "달라" 같은 단순한 답변만 있는 경우
        ...
        [작성할 보고서 내용]
        위 [학생 활동 요약]을 [성취 기준]과 [평가 기준]에 엄격하게 근거하여, 다음 3가지 항목으로 나누어 객관적이고 상세하게 평가 보고서를 작성해주세요. (HTML <p>, <ul>, <li>, <strong> 태그를 사용하여 가독성 좋게 작성)

        1.  **종합 평가 (성취 수준):** 
            * 학생의 실제 활동 내용을 정확히 확인하여 성취 수준을 '잘함', '보통', '노력 요함' 중 하나로 결정
            * 특히 읽기 중 활동의 질문과 답의 내용을 실제로 확인하여 평가 (무조건 잘함이 아님)
            * 결정한 이유를 구체적인 근거와 함께 <strong> 태그로 강조하여 설명
            * 활동 내용이 부족하면 솔직하게 지적
            
        2.  **단계별 상세 평가:**
            * '읽기 전', '읽기 중', '읽기 후' 각 단계별로 학생의 활동(v1)이 글의 종류(${articleType})에 맞게 잘 이루어졌는지 상세히 평가
            * 읽기 중 활동: 질문의 종류와 내용, 답의 내용을 실제로 확인하여 평가 (예: "몰라요" 같은 답은 부족함을 지적)
            * '읽기 조정' 활동에 대해서도 평가
            * 각 단계의 부족한 점을 구체적으로 지적
            
        3.  **성장 과정 (스스로 조정하기):**
            * 학생이 AI 피드백을 받고 '수정한 내용(v2)'이 '처음 작성한 내용(v1)'에 비해 어떻게 발전했는지 구체적으로 비교
            * v2가 v1보다 나아졌다면 칭찬하되, 여전히 부족한 점이 있으면 지적
            * v2가 v1과 같거나 개선이 없다면, 그 점을 명확히 지적
            * 이것이 "스스로 조정"하는 '잘함' 수준에 해당하는지 엄격하게 판단하여 명시
            * 수정하지 않았다면, 그 점을 명확히 언급
    `;
